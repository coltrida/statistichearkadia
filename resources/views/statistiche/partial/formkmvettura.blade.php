<form action="{{route('visualizza_chilometri_vetture')}}" method="POST">
    @csrf
    <div class="row">
        <div class="col form-group">
            <label for="vettura" class="text-white">Vettura</label>
            <select required class="custom-select" name="vettura" id="vettura">
                <option value=""></option>
                @foreach($vetture as $vettura)
                    <option value={{$vettura->id}}>{{$vettura->name}}</option>
                @endforeach
            </select>
        </div>
    </div>
